...
class StartHostTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler, config_manager, iso_manager, service_manager):
        ...
        BaseTask.__init__(self, task_type, RequestDefine.start_host, messsage_handler, logger_name)

        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.start_host, result_success, self.onStartSuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.start_host, result_fail, self.onStartFail)
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X] <start_host> receive request from '%s', host id '%s', boot '%d'" %
                       (session.session_id, session.request_module, uuid, boot))

        if not self.config_manager.containsHost(uuid):
            self.error("[%08X] <start_host> fail, invalid host id '%s'" %
        ...
        if not self.service_manager.containsService(node_client_name):
            self.error("[%08X] <start_host> fail, host container '%s' does not exist" %
        ...
        if 1 == boot:
            ...
            if not self.iso_manager.containsImage(image_id):
                self.error("[%08X] <start_host> fail, invalid boot iso '%s'" %
            ...
            if image.disk_type != node_client.disk_type:
                self.error("[%08X] <start_host> fail, disk type not match, iso image '%s/%d', node client '%s/%d'" %
            ...
            if image.disk_type == ComputeStorageTypeEnum.local:
                self.info("[%08X] <start_host> boot from iso '%s', address '%s:%d'" %
                ...
                self.info("[%08X] <start_host> boot from iso '%s', path '%s'" %
                           (session.session_id, image.name, image.path))
                request.setString(ParamKeyDefine.path, image.path)

        self.info("[%08X] <start_host> redirect start host '%s'('%s') to compute node '%s'" %
    ...
    def onStartSuccess(self, msg, session):
        ...
        self.info("[%08X] <start_host> success, id '%s'" %
    ...
    def onStartFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X] <start_host> fail, id '%s'" %
                  (session.session_id, session.target))
        self.taskFail(session)

    def onStartTimeout(self, msg, session):
        self.info("[%08X] <start_host> timeout, id '%s'" %
                  (session.session_id, session.target))
        self.taskFail(session)
