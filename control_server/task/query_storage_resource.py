...
class QueryStorageResourceTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler, storage_manager):
        ...
        logger_name = "task.query_storage_resource"
        BaseTask.__init__(self, task_type, RequestDefine.query_storage_resource, messsage_handler, logger_name)
    ...
    def invokeSession(self, session):
        self.info("[%08X]receive query storage resource request from '%s'." % (session.session_id, session.request_module))
    ...
    def onQuerySuccess(self, response, session):
        ...
        self.info("[%08X]query storage resource success." % (session.session_id))
    ...
    def onQueryFail(self, response, session):
        self.clearTimer(session)
        self.error("[%08X]query storage resource fail." % (session.session_id))
        self.taskFail(session)
        
        
    def onQueryTimeout(self, response, session):
        self.clearTimer(session)
        self.error("[%08X]query storage resource timeout." % (session.session_id))
        self.taskFail(session)
