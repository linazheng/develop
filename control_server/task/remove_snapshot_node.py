...
class RemoveSnapshotNodeTask(BaseTask):
    ...
    def __init__(self, task_type, message_handler, snapshot_pool_manager):
        ...
        logger_name = "task.remove_snapshot_node"
        BaseTask.__init__(self, task_type, RequestDefine.remove_snapshot_node, message_handler, logger_name)

        # state inital
        self.addTransferRule(state_initial,
                             AppMessage.RESPONSE,
                             RequestDefine.remove_snapshot_node,
                             result_success,
                             self.onSuccess,
                             state_finish
                             )
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X]receive remove snapshot node request from '%s', pool '%s', node '%s'" %
    ...
    def onSuccess(self, response, session):
        ...
        node = request.getString(ParamKeyDefine.name)
        self.snapshot_pool_manager.removeSnaspshotNode(snapshot_pool, node)
        self.info("[%08X]remove snapshot node success. pool '%s', node '%s'" %
    ...
    def onFail(self, response, session):
        ...
        snapshot_pool = request.getString(ParamKeyDefine.pool)
        node = request.getString(ParamKeyDefine.name)
        self.error("[%08X]remove snapshot node fail. pool '%s', node '%s'" %
                   (session.session_id, snapshot_pool, node))
        self.taskFail(session)
    ...
    def onTimeout(self, response, session):
        ...
        snapshot_pool = request.getString(ParamKeyDefine.pool)
        node = request.getString(ParamKeyDefine.name)
        self.info("[%08X]remove snapshot node timeout. pool '%s', node '%s'" %
                  (session.session_id, snapshot_pool, node))
        self.taskFail(session)
