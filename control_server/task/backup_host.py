...
class BackupHostTask(BaseTask):
    ...
    def __init__(self, task_type, message_handler, config_manager):
        logger_name = "task.backup_host"
        self.config_manager = config_manager
        BaseTask.__init__(self, task_type, RequestDefine.backup_host, message_handler, logger_name)
        ...
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.backup_host, result_fail, self.onFail)
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.backup_host, result_success, self.onSuccess)
        ...
        self.addTransferRule(state_operate, AppMessage.RESPONSE, RequestDefine.backup_host, result_fail, self.onFail)
        self.addTransferRule(state_operate, AppMessage.RESPONSE, RequestDefine.backup_host, result_success, self.onSuccess)
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X]receive backup host request, host id '%s', mode '%d', disk '%d'" %
                  (session.session_id, uuid, mode, disk))

        # uuid check
        if not self.config_manager.containsHost(uuid):
            self.error("[%08X]backup host fail, host id '%s' does not exist" %
        ...
        if mode not in (BackupHostModeEnum.fully, BackupHostModeEnum.partial):
            self.error("[%08X]backup host fail, no support mode '%d'" %
                       (session.session_id, mode))
            self.taskFail(session)
            return

#         # host enable backup check
#         if host_info.enable_local_backup == EnableLocalBackupEnum.disabled:
#             self.error("[%08X]backup host fail, host '%s' does not support backup." %
#                        (session.session_id, uuid))
#             self.taskFail(session)
#             return
...
#         request = session.initial_message
#         uuid = request.getString(ParamKeyDefine.uuid)
#         self.info("[%08X]<backup_host>backup host start, host id '%s'..." %
#                   (session.session_id, uuid))
    ...
    def onFail(self, msg, session):
        ...
        mode = request.getUInt(ParamKeyDefine.mode)
        disk = request.getUInt(ParamKeyDefine.disk)
        self.error("[%08X]backup host fail, host '%s' in mode '%d' at disk %d'" %
                   (session.session_id, uuid, mode, disk))
    ...
    def onSuccess(self, msg, session):
        ...
        mode = request.getUInt(ParamKeyDefine.mode)
        disk = request.getUInt(ParamKeyDefine.disk)
        self.info("[%08X]backup host success, host '%s' in mode '%d' at disk '%d'" %
                  (session.session_id, uuid, mode, disk))
    ...
    def onTimeout(self, msg, session):
        ...
        mode = request.getUInt(ParamKeyDefine.mode)
        disk = request.getUInt(ParamKeyDefine.disk)
        self.error("[%08X]backup host timeout, host '%s' in mode '%d' at disk '%d'" %
                   (session.session_id, uuid, mode, disk))
...
#         
#         self.info("[%08X]<backup_host>backup host '%s' at progress %d%%..." %
