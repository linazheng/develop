...
class ChangeMediaTask(BaseTask):
    ...
    def __init__(self, task_type, messsage_handler, config_manager, iso_manager, service_manager):
        ...
        BaseTask.__init__(self, task_type, RequestDefine.change_media, messsage_handler, logger_name)

        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.change_media, result_success, self.onChangeSuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.change_media, result_fail, self.onChangeFail)
        self.addTransferRule(state_initial, AppMessage.EVENT, EventDefine.timeout, result_any, self.onChangeTimeout)
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X] <change_media> receive request from '%s', host id '%s'" %
                       (session.session_id, session.request_module, uuid))

        if not self.config_manager.containsHost(uuid):
            self.error("[%08X] <change_media> fail, invalid id '%s'" %
        ...
        if not self.service_manager.containsService(node_client_name):
            self.error("[%08X] <change_media> fail, host container '%s' does not exist" %
        ...
        if not self.iso_manager.containsImage(image_id):
            self.error("[%08X] <change_media> fail, invalid iso id '%s'" %
        ...
        if image.disk_type != node_client.disk_type:
            self.error("[%08X] <change_media> fail, disk type not match, iso image '%s/%d', node client '%s/%d'" %
        ...
        if image.disk_type == ComputeStorageTypeEnum.local:  # #image name, ip, port
            self.info("[%08X] <change_media> change iso media '%s' into host '%s', address '%s:%d'" %
            ...
            self.info("[%08X] <change_media> change iso media '%s' into host '%s', path '%s'" %
                       (session.session_id, image.name, host.name, image.path))
            request.setString(ParamKeyDefine.path, image.path)

        self.info("[%08X] <change_media> redirect change media '%s'('%s') to compute node '%s'" %
    ...
    def onChangeSuccess(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X] <change_media> success, iso id '%s'" %
    ...
    def onChangeFail(self, msg, session):
        self.clearTimer(session)
        self.info("[%08X] <change_media> fail, iso id '%s'" %
                  (session.session_id, session.target))
        self.taskFail(session)

    def onChangeTimeout(self, msg, session):
        self.info("[%08X] <change_media> timeout, iso id '%s'" %
                  (session.session_id, session.target))
        self.taskFail(session)
