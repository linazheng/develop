...
class QueryAddressResourceTask(BaseTask):
    def __init__(self, task_type, messsage_handler,
        ...
        logger_name = "task.query_address_resource"
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X]receive query address resource request from '%s', pool '%s'"%(
                session.session_id, session.request_module,
                pool_id))

        resource_list = self.address_manager.queryResource(pool_id)
        if 0 == len(resource_list):
            self.error("[%08X]query address resource fail, no address resource available"%
                       session.session_id)
        ...
        self.info("[%08X]query address resource success, %d address resource(s) available"%
                  (session.session_id, len(resource_list)))
        
        response = getResponse(RequestDefine.query_address_resource)
