...
class DisableStorageDeviceTask(BaseTask):
    ...
    def __init__(self, task_type, message_handler, service_manager):
        ...
        BaseTask.__init__(self, task_type, RequestDefine.disable_storage_device, message_handler, logger_name)

        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.disable_storage_device, result_success, self.onSuccess)
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.disable_storage_device, result_fail, self.onFail)
    ...
    def invokeSession(self, session):
        ...
        self.info("[%08X] <disable_storage_device> receive request, level '%d', target '%s', disk_type '%d', index '%d'" %
                  (session.session_id, level, target, disk_type, index))

        if level != 1:
            self.warn("[%08X] <disable_storage_device> disable fail, no support level '%d'" %
                      (session.session_id, level))
            self.taskFail(session)
            return

        if not self.service_manager.containsService(target):
            self.error("[%08X] <disable_storage_device> disable fail, invalid target '%s'" %
        ...
        if disk_type not in (ComputeStorageTypeEnum.local, ComputeStorageTypeEnum.cloud, ComputeStorageTypeEnum.nas, ComputeStorageTypeEnum.ip_san):
            self.error("[%08X] <disable_storage_device> disable fail, invalid disk type '%d'" %
    ...
    def onSuccess(self, msg, session):
        ...
        self.info("[%08X] <disable_storage_device> success" %
                  (session.session_id))
    ...
    def onFail(self, msg, session):
        self.clearTimer(session)
        self.error("[%08X] <disable_storage_device> fail" %
                   (session.session_id))
        self.taskFail(session)

    def onTimeout(self, msg, session):
        self.error("[%08X] <disable_storage_device> timeout" %
                   (session.session_id))
        self.taskFail(session)
