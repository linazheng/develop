...
class DeleteSnapshotTask(BaseTask):
    ...
    def __init__(self, task_type, message_handler, config_manager):
        ...
        logger_name = "task.delete_snapshot"
        BaseTask.__init__(self, task_type, RequestDefine.delete_snapshot, message_handler, logger_name)
        
        # state inital
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.delete_snapshot, result_success, self.onSuccess, state_finish );
        self.addTransferRule(state_initial, AppMessage.RESPONSE, RequestDefine.delete_snapshot, result_fail,    self.onFail,    state_finish );
    ...
    def invokeSession(self, session):
        ...
        v_snapshot = request.getString(ParamKeyDefine.snapshot)
        ...
        self.info("[%08X] <delete_snapshot> receive delete snapshot request from '%s', parameter: %s" % (session.session_id, session.request_module, dict_util.toDictionary(v_parameter)))
        
        # instance of HostInfo
        host = self.config_manager.getHost(v_target)
        if host is None:
            self.error("[%08X] <delete_snapshot> delete snapshot fail, invalid id '%s'" % (session.session_id, v_target))
            self.taskFail(session)
            return
        
        # 
        self.info("[%08X] <delete_snapshot> send delete_snapshot request to node_client '%s'" % (session.session_id, host.container))
    ...
    def onSuccess(self, response, session):
        ...
        self.info("[%08X] <delete_snapshot> delete snapshot for host '%s' success." % (session.session_id, session._ext_data["parameter"]["target"]));
    ...
    def onReport(self, response, session):
        ...
        self.info("[%08X] <delete_snapshot> report deleting snapshot for host '%s' at level '%s'." % (session.session_id, session._ext_data["parameter"]["target"], v_level))


    def onFail(self, response, session):
        self.clearTimer(session)
        self.info("[%08X] <delete_snapshot> delete snapshot for host '%s' fail." % (session.session_id, session._ext_data["parameter"]["target"]))
        response.session = session.request_session
        self.sendMessage(response, session.request_module)
        session.finish()
    ...
    def onTimeout(self, response, session):
        self.clearTimer(session)
        self.info("[%08X] <delete_snapshot> delete snapshot for host '%s' timeout." % (session.session_id, session._ext_data["parameter"]["target"]))
        response.session = session.request_session
        self.sendMessage(response, session.request_module)
        session.finish()
