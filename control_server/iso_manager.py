...
class ISOManager(object):  
    ...
    def loadImages(self, container_name, image_list):
        with self.lock:
            ...
            self.logger.info("<iso_manager> %d iso image(s) loaded in container '%s'"%(
    ...
    def addImage(self, image):
        with self.lock:
            if self.image_names.has_key(image.name):
                self.logger.error("<iso_manager> add iso image fail, image name '%s' already exists"%(
                    image.name))
                return False
            if self.images.has_key(image.uuid):
                self.logger.error("<iso_manager> add iso image fail, image id '%s' already exists"%(
                    image.uuid))
                return False
            self.images[image.uuid] = image
            self.image_names[image.name] = image.uuid
            ...
            self.logger.info("<iso_manager> add iso image success, name '%s'('%s'), container '%s'"%(
    ...
    def removeImage(self, uuid):
        with self.lock:
            if not self.images.has_key(uuid):
                self.logger.error("<iso_manager> remove iso image fail, image '%s' not exists"%(
                    uuid))
                return False
            image = self.images[uuid]
            ...
            if self.image_containers.has_key(container_name):
                ...
                if uuid in id_list:
                    ...
                    self.logger.info("<iso_manager> remove iso image '%s' from container '%s'"%(
                        uuid, container_name))
            self.logger.info("<iso_manager> remove iso image success, name '%s'('%s'), container '%s'"%(
    ...
    def modifyImage(self, uuid, new_image):
        with self.lock:
            if not self.images.has_key(uuid):
                self.logger.error("<iso_manager> modify iso image fail, image '%s' not exists"%(
                    uuid))
                return False
            image = self.images[uuid]        
            ...
            self.logger.info("<iso_manager> modify iso image success, name '%s'('%s')"%(
                image.name, image.uuid))    
            return True

    def removeAllImageInContainer(self, container_name):
        self.logger.info("<iso_manager> remove all images in container '%s'" %
                         (container_name))
